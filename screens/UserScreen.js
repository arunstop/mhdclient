  ...
  const mapData = data.map((item, index) => {
    return (
      <DataTable.Row key={index}>
        ...
        <DataTable.Cell style={styles.tableMargin}>{item.FIRST_NAME + " "+item.LAST_NAME}</DataTable.Cell>
        <DataTable.Cell style={styles.tableMargin}>{item.EMAIL}</DataTable.Cell>
        <DataTable.Cell style={styles.tableMargin}>
          <FontAwesome5 name="eye" size={12} color="blue" />
          <Link style={styles.link} target="_blank" to={item.PHOTO_URL}>
            Thumbnail
          </Link>
        </DataTable.Cell>
        <DataTable.Cell>
          <View style={styles.btnAction}>
            <Button title="NON-AKTIFKAN" color="tomato" onPress={() => { initDelete(item.ID_USER) }} />
          </View>
        </DataTable.Cell>
          ...
          <View style={{ marginTop: 24 }}>
            <DataTable  >
              <DataTable.Header style={styles.tableHeader}>
                ...
                <TableTitle style={styles.tableMargin} title="Email" />
                <TableTitle style={styles.tableMargin} title="Name" />
                <TableTitle style={styles.tableMargin} title="Foto Profil" />
